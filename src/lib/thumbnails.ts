...
/**
 * Скачивает превью по URL и сохраняет в {DOWNLOAD_PATH}/thumbnails/{platformId}.{ext}.
 * Возвращает относительный путь (например thumbnails/xxx.jpg) или null при ошибке.
 */
export async function downloadAndSaveVideoThumbnail(
  thumbnailUrl: string,
  platformId: string,
  getDownloadPath: () => Promise<string>
): Promise<string | null> {
  try {
    const res = await fetch(thumbnailUrl, { signal: AbortSignal.timeout(15000) });
    ...
    const dir = path.join(root, THUMBNAILS_DIR);
