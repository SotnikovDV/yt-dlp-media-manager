...
- При блокировке экрана или сворачивании браузера **решение о паузе воспроизведения принимает сама ОС/браузер** — веб-приложение не может принудительно запретить авто-паузу.
- Непрерывное воспроизведение «как у нативных аудио‑плееров» при выключенном экране в большинстве случаев **недоступно для обычного сайта** по соображениям экономии батареи и лицензирования контента.
- Для более предсказуемого фонового поведения рекомендуется:
  - Использовать приложение как PWA («Добавить на главный экран» в браузере).
  - При необходимости полного фонового воспроизведения рассматривать отдельный нативный Android‑клиент, использующий фоновые сервисы и системный плеер.
