...
- 📺 **Медиатека** — просмотр и организация скачанных видео (избранное, закреплённые, просмотры, теги, плейлисты)
- 📥 **Скачивание** — загрузка видео с выбором качества и формата; скачивание аудио (AAC, `.m4a`) с перекодированием на сервере
- 🔔 **Подписки** — автоматическое отслеживание новых видео на каналах (категории, расписание)
- 📋 **Очередь** — управление задачами скачивания
- 🎬 **Видеоплеер** — встроенный плеер (видео/аудио, мини-режим, прогресс), трансляция на Chromecast, публичные ссылки просмотра
...
│   │   │   ├── videos/        # Управление видео, секции, избранное, просмотры, аудио (AAC)
│   │   │   ├── download/      # Скачивание
│   │   │   ├── queue/         # Очередь задач
...
│   │   │   ├── settings/      # Настройки
│   │   │   └── ...
│   │   ├── (main)/            # Основной layout; [tab] — вкладки (медиатека, подписки, очередь, настройки)
...
│   │   ├── ffmpeg-extract-audio.ts  # Извлечение аудио в AAC
│   │   ├── prepared-audio-download.ts # Клиент: скачивание аудио
...
| GET | /api/videos/[id]/audio?download=1 | Скачать аудио в AAC (.m4a) |
...
| AUDIO_EXTRACT_AAC_BITRATE | Битрейт для извлечения аудио в AAC (96k, 128k и т.п.) | 96k |
| AUDIO_EXTRACT_AAC_MONO | Моно (1/true) или стерео (0/false) для AAC | 0 |
| NEXT_PUBLIC_AUDIO_DOWNLOAD_MAX_CONCURRENT | Макс. одновременных «скачать аудио» в браузере | 1 |
...
- **NEXTAUTH_URL** и **BASE_URL** в production укажите на реальный адрес приложения (например, [`https://your-domain.com`](https://your-domain.com)).
...
### Скачивание аудио (AAC)

Из карточки видео или окна описания доступно действие «Скачать» → «Аудио». Сервер извлекает аудиодорожку через ffmpeg в формат AAC (`.m4a`). Битрейт и моно/стерео задаются в Настройках (админ) или через `AUDIO_EXTRACT_AAC_BITRATE`, `AUDIO_EXTRACT_AAC_MONO` в `.env`. Во время подготовки заголовок «DVStream» пульсирует и показывается счётчик фоновых задач.

### Фоновое воспроизведение и мобильные браузеры

- Встроенный видеоплеер использует стандартный HTML5 `<video>` и Media Session API для интеграции с системными медиакнопками (шторка уведомлений, экран блокировки на Android).
- Для более предсказуемого фонового поведения рекомендуется использовать приложение как PWA («Добавить на главный экран» в браузере).
